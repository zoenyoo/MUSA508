...
ggplot() +
  ...
  geom_sf(data = boston.sf, aes(color = q5(PricePerSq)), 
...
bostonCrimes.sf <-
  bostonCrimes %>%
    ...
    na.omit() %>% #remove NA
    st_as_sf(coords = c("Long", "Lat"), crs = 4326) %>% #also included , agr = "constant"
    st_transform('ESRI:102286') %>%
...
boston.sf <-
  boston.sf %>% 
    mutate(
      ...
      crime_nn4 = nn_function(st_c(boston.sf), st_cg(bostonCrimes.sf), 4), 
...
```{r crime_corr, fig.height=4, fig.width=16}
## Crime cor
boston.sf %>%
  st_drop_geometry() %>%
...
cor.test(boston$LivingArea, boston$SalePrice, method = "pearson") #p-value = probability that is random 
...
```{r simple_reg}
livingReg <- lm(SalePrice ~ LivingArea, data = boston.sf)
