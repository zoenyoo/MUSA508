...
palette <- c("#C81D77","#C11C7D","#B91B83","#B21A88","#AA198E","#A31894","#9B179A","#94169F","#8C15A5","#8514AB","#7D13B1","#7612B6","#6E11BC","#6710C2")
...
ggplot()+
  ...
  scale_fill_manual(values=palette, aesthetics="fill")+
  mapTheme()
```
...
Here is a basic plot of the buffers that we have created.

```{r buffer_plot}
ggplot() + 
  geom_sf(data = bothBuffers, aes(fill = Legend)) +
  scale_fill_manual(values = c("#F28468", "#7350CF")) + 
...
ggplot() +
  geom_sf(data=st_union(studyAreaTowns))+
  geom_sf(data=buffersAndTowns %>% dplyr::filter(Legend == "Inside"), aes(fill=MUNI), alpha = 1, show.legend = FALSE) +
  geom_sf(data=buffersAndTowns %>% dplyr::filter(Legend == "Outside"), aes(fill=MUNI), alpha = 0.3, show.legend = FALSE) +
  #geom_sf_text(data = studyAreaTowns %>% dplyr::filter(MUNI != "MOUNTVILLE BOROUGH"), 
  #                                  aes(label = MUNI), size = 3) +
  geom_sf_label(data=buffersAndTowns %>% dplyr::filter(MUNI != "MOUNTVILLE BOROUGH"), aes(label= str_remove(MUNI, "TOWNSHIP")), size = 2) +
  theme_classic()
  
  #str_trunc(studyAreaTowns$, width, side = c("right", "left", "center"), ellipsis = "...")
```