...
ggplot() +
  ...
  scale_fill_viridis() +
  labs(title = "Count of _ for the fishnet") +
  mapTheme()
...
crashes19 <- #vehicle crashes just in 2019 -- doesn't work
  read.csv("https://raw.githubusercontent.com/zoenyoo/MUSA508/main/Labs/MUSA_508_Lab-main/Traffic_Crashes_-_Vehicles.csv") %>% 
    mutate(x = gsub("[()]", "", LOCATION)) %>%
    separate(x,into= c("Y","X"), sep=",") %>%
    mutate(X = as.numeric(X),Y = as.numeric(Y)) %>% 
    subset(!is.na(X)) %>%
...
liquor <- #all licenses
  ...
  mutate(x = gsub("[()]", "", LOCATION)) %>%
    ...
    st_transform('ESRI:102271') %>% 
    distinct() %>% 
  mutate(Legend = "Liquor_Est")

graffiti <- #just between 2018-19
  ...
  mutate(x = gsub("[()]", "", LOCATION)) %>%
    ...
    st_transform('ESRI:102271') %>% 
    distinct() %>% 
  mutate(Legend = "Graffiti_311")

speedcamv <-
  read.csv("https://raw.githubusercontent.com/zoenyoo/MUSA508/main/Labs/MUSA_508_Lab-main/Speed_Camera_Violations_2019.csv") %>% 
  ...
  mutate(x = gsub("[()]", "", LOCATION)) %>%
    ...
    st_transform('ESRI:102271') %>% 
    distinct() %>% 
  mutate(Legend = "Speed_Cam_Violations")

redlightv <-
  read.csv("https://raw.githubusercontent.com/zoenyoo/MUSA508/main/Labs/MUSA_508_Lab-main/Red_Light_Camera_Violations.csv") %>% 
  #https://data.cityofchicago.org/Transportation/Red-Light-Camera-Violations/spqx-js37/data
  mutate(x = gsub("[()]", "", LOCATION)) %>%
    separate(x,into= c("Y","X"), sep=",") %>%
    mutate(X = as.numeric(X),Y = as.numeric(Y)) %>% 
    na.omit() %>%
    ...
    st_transform('ESRI:102271') %>% 
    distinct() %>% 
  mutate(Legend = "Red_Light_Violations")



majorst <-
  read_sf("./Major_Streets/Major_Streets.shp") %>% 
  st_transform('ESRI:102271') %>% 
  distinct() %>% 
  mutate(Legend= "Major Streets")
...
ggplot() +
  geom_sf(data=chicagoBoundary, show.legend = NA, color='black', lwd=0.1, alpha=.3)+
  geom_sf(data = crimes, aes(), 
          show.legend = "point", size = 0.3, color='red')+
  mapTheme()+
  labs(title="2019 _ Crimes", 
       subtitle="Chicago, IL", 
       caption="Figure 2.4")

ggplot()+
  geom_sf(data=chicagoBoundary, show.legend = NA, color='black', lwd=0.1, alpha=.3)+
  geom_sf(data = crimes20, aes(), 
          show.legend = "point", size = 0.3, color='red')+
  mapTheme()+
  labs(title="2020 _ Crimes", 
       subtitle="Chicago, IL", 
       caption="Figure 2.4")
...
vars_net <- vars_net %>%
  ...
  mutate(graffiti.nn = nn_function(st_c(st_coid(vars_net)), 
                                           st_c(graffiti),
                                           k = 3)) %>% 
  mutate(speedcam.nn = nn_function(st_c(st_coid(vars_net)), 
                                           st_c(speedcamv),
                                           k = 3))
...
```{r}
## see ?localmoran
local_morans <- localmoran(final_net$crimes, final_net.weights, zero.policy=TRUE) %>% 
  as.data.frame()
...
final_net.localMorans <- 
  ...
  st_sf() %>%
  dplyr::select(Crime_Count = crimes, 
                Local_Morans_I = Ii, 
                P_Value = `Pr(z != E(Ii))`) %>%
...
do.call(grid.arrange,c(varList, ncol = 4, top = "Local Morans I statistics, _"))
```
...
ggplot(data=crim_KD.df, aes(x=x, y=y)) +
...
as.data.frame(burg_KD.1000) %>%
  ...
  aggregate(., final_net, mean) %>%
   ggplot() +
     ...
     scale_fill_viridis(name = "Density") +
     labs(title = "Kernel density of 2017 _") +
     mapTheme(title_size = 14)
```
